...
#updated names in the dataset
updated = ["Ahmedabad", "Road", "NR", "Avenue", "SBK", "Gandhi", "Bridge", "Society", "Four", "'"] 
...
# Search string for the reg. If matched and not in the updated list, then add it as a key to the set.
def audit_street(street_types, street_name): 
    m = reg.search(street_name)
    if m:
        street_type = m.group()
        if street_type not in updated:
